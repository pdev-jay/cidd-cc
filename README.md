...
- **plan 기계**: `friction-extractor`, `plan-reviser`, `completeness-critic`(사각지대 — 안 본 관심사), `assumption-critic`(숨은 전제 — 비교 없이 단일 plan에 직접 질문, 옛 `direction-explore`가 후보 비교로만 얻던 값을 대체)
...
target_repo: <path>
stage:  plan | build | review | done   # = 여기까지 *승인*됨(다음이 열림). 강제 아님 — ratchet 아니다.
...
handoff:
  ...
  build→review: adequacy 미측정 unit · conformance 우려 · PROD PATH UNVERIFIED 라인  → review 집중점
...
- **단계 끝 = 결정 메뉴(고르면 자동 처리).** 각 스킬은 산출물을 낸 뒤 `AskUserQuestion`으로 *현재 stage에 유효한* 선택지를 띄우고, 고른 전이를 **state.md에 자동 적용**한다 — 수동 편집·별도 호출 없음. 상태는 두 축: **stage**(어디까지) × **status**(active/paused/abandoned). abandon·done은 status지 accept/refine/back과 동급이 아니다.
  ...
  | **back** | stage→이전(또는 지정한 더 앞) | plan 외 |
  ...
  - "끝까지 알아서"는 accept를 기본 자동 전진하는 **오토 모드**(`/cidd:auto`). **오토 모드에선 단계-끝 메뉴를 생략**하고 green에 자동 전이 — 메뉴는 fork·red·done에서만(green 경계서 "계속?" 묻지 않음). 기본(비-auto)은 이 메뉴(사람 게이트).
...
## 인라인 출력 형태
인라인(대화) 출력은 **결과물 우선** — 그 단계 산출물(plan=결론 plan / build=빌드 상태 / review=머지 판정)을 다이어그램+핵심으로 보여주고, 과정(라운드·repair raw)은 단계 파일(`runs`/`builds`/`reviews`)로 강등한다. 진짜 fork/트레이드오프가 남으면 "결정 포인트"로 묻는다. 평이어(내부용어는 ≤6단어로 풀기)·신호밀도(장식기호·과압축 금지)·적응형(빈 섹션 생략) 게이트. **규약 본문은 각 스킬 출력 절이 contained로 담는다** — 스킬은 런타임에 README를 로드하지 않으므로 참조가 아니라 내장이다.
...
- **방향 불확실성 → 심의**(lens·라운드·judge). 뻔하면 0까지 접는다 — 비용 대부분이 여기서 빠진다.
...
## 설치 / 사용
Claude Code `Agent` 도구로 동작. 서브에이전트 모델은 작업에 맞춰 haiku(grounded·enumeration) / sonnet(grounding 없는 판단 — plan 판단 lens·plan-reviser·assumption-critic·oracle-runner) / 상속(생성 — builder), low effort.
...
출력은 대상 프로젝트 cwd의 `.cidd/`에 모인다 — 결과물 `.cidd/plans/`·`.cidd/builds/`·`.cidd/reviews/`(써먹는 산출물; build은 코드 변경은 repo에, 리포트는 builds/), 과정 로그 `.cidd/runs/`(감사·dogfooding). 단계 연결: plan-friction-loop이 부트스트랩으로 `plans/<slug>.md`를 만듦 → build이 소비·diff 생성 → review가 그 diff에 오라클. 네임스페이스라 남의 repo와 충돌 없음, `.gitignore`에 `.cidd/` 한 줄.
