...
## 입력
- **plan**: 검토할 구현 계획 (텍스트 또는 파일 경로). 없으면 goal만 갖고 아래 "목표 명확화"부터 거쳐라.
- **repo (선택)**: 대상 코드베이스 경로. 주어지면 ground-truth 검증이 켜진다 — 서브에이전트 프롬프트에 repo 경로를 넘겨, 가정을 코드로 확인하게 하라.

## 목표 명확화 (plan 초안 쓰기 전 — goal만 있고 plan이 없을 때)
이 스킬은 부트스트랩이라 종종 초안 없이 goal 한 줄만 들어온다. 초안을 쓰기 전에 먼저 이 게이트를 거쳐라. **서브에이전트 없이 너(메인 에이전트) 혼자, 비용 0.** 목적은 **goal·repo 어디에도 없어서 기계로는 못 채우는 것**을 사람에게서 뽑는 것뿐 — repo가 답하는 걸 사람에게 되묻는 건 노이즈다(오라클 빼기를 *사람 질문*에도 적용).
1. 얕은 repo scan(있으면) 먼저 — grep 한 번이면 풀리는 건 질문 후보에서 빼라. **goal·repo가 이미 답하는 건 절대 되묻지 마라.**
2. 남은 것 중 아래 둘 중 하나라도 걸리면 물어라(대부분의 요청은 안 걸린다 → 3으로):
   - **(A) 구조 가르는 다중해석** — goal이 여러 갈래로 읽히고 그 차이가 plan 핵심 구조를 가른다. 세부 파라미터 수준 모호함(폴링 주기·로그 포맷)은 제외 — lens가 처리.
   - **(B) material unknown-knowns** — goal·repo엔 없지만 *사용자 머릿속엔 있을* 암묵 맥락 중, 틀리게 가정하면 plan 방향이 바뀌는 것(예: 이 서비스들이 별도 배포 유닛인가, 즉시성이 컴플라이언스 요건인가 — repo가 안 말해주는 도메인 사실). ⚠️ "내가 모르는데 사용자는 아는 것"을 겨눈다 — **낯선 도메인일수록 크다**. `assumption-critic`(뒷단·기계)이 못 닿는 유일한 칸이라 여기서만 잡힌다.
3. 아니면(대부분) 그냥 초안 한 문단을 써서 바로 아래 절차로 진행해라.
4. 걸리면 `AskUserQuestion`으로 물어라. **기본은 한 라운드 배치**(독립 질문은 한 번에 묶어라). ⚠️ **단 도메인이 너에게 낯설어(B가 크고 unknown-unknown 위험) 한 답이 다음 질문을 여는 경우엔 적응형 다회 허용** — 각 답이 다음을 실제로 좁힐 때만, 캡 3라운드. 익숙한 도메인에서 왕복을 늘리지 마라(노이즈). 답으로 goal을 확정한 뒤 초안을 써서 진행해라. (전이 확인 왕복 — "다음 단계 갈까?" — 은 이 게이트가 아니라 단계-끝 결정 메뉴 몫이다.)
- ⚠️ **`clarify` 스킬이 앞서 돌아 enriched goal이 넘어왔으면 이 게이트는 no-op**(사람 인터뷰를 이미 깊게 했으니 중복 금지) — enriched goal 그대로 초안으로 진행. (이 게이트는 clarify를 *안* 부른 경우의 얕은 자동 안전망이다.)
- ⚠️ **단 `auto`가 구동 중이면 이 게이트를 생략**하고, auto의 시작 1회 동의(`AskUserQuestion`)에 (A)·(B) 질문을 배치로 얹은 것으로 간주하라 — auto는 그 이후 안 멈추니 적응형 다회는 auto에선 불가(시작 1회에 몰아라).
- 이건 **요청·맥락의 앞단 모호성**(요청 자체 + 사람 암묵 맥락)을 사람에게 물어 잡는다. plan을 다 쓴 뒤에야 드러나는 숨은 전제는 이 게이트가 아니라 `assumption-critic`(수렴 후·기계가 plan 심문) 몫 — 둘은 대체가 아니라 앞단(사람)/뒷단(기계) 짝이다.

## 단계 상태 (lifecycle 척추)
이 스킬은 plan 단계이자 **파이프라인의 부트스트랩**(앞 단계 없음). 완료 시 대상 repo의 `.cidd/state.md`에 work item을 등록/갱신한다(스키마는 README "단계 상태"): `active: <slug>`, `target_repo`, `plan: done → .cidd/plans/<slug>.md`, `stage: build`(다음이 *열림* — 강제 아님), `handoff.plan→build`(scope · 미해결/검증필요 항목). 같은 slug 재실행이면 기존 항목 갱신. ⚠️ **plan이 마음에 안 들면**: 재실행(같은 방향 다듬기) / **abandon**(방향 자체가 틀렸을 때 — 되돌아갈 이전 단계가 없다). **끝나면 `AskUserQuestion`으로 결정 메뉴(accept/refine/pause/abandon)를 띄우고 고른 전이를 자동 적용한다**(README "단계 끝 = 결정 메뉴") — 강제 전진 아님. ⚠️ **단 `auto`가 구동 중이면 이 끝-메뉴를 생략하고 전이 신호(수렴 done / 미해소 fork)만 반환하라** — auto가 전이를 소유하고 메뉴는 fork·done에서만 띄운다. (`updated` 날짜는 세션이 박는다.)
...
- **규모는 state.md의 `profile` 필드가 있으면(예: `auto`가 전역 triage로 박아둔 값) 그걸 출발점으로 써라.** 없으면(이 스킬은 파이프라인 부트스트랩이라 — 앞 단계가 없어 incoming handoff 자체가 없다) 자가 판정: ① 방향 뻔한가 ② 틀리면 위험한가 ③ 관련 테스트 있나 — 요청 형용사보다 얕은 repo scan 우선(서브에이전트 X).
...
- **task-gated**: `lens-testability`(side-effect·시간·IO·전역·주입 장애물이 *미해결*로 닿을 때 — 장애물이 없거나 이미 해결돼 있으면 고유 finding 0, 미해결일 때만 관측-seam 같은 고유값 [측정 3 task]), `lens-security`(auth/입력), `lens-operability`(배포), `lens-cost`(성능)
...
6. **비평 fan-out (병렬).** 최종 `plan`을 두 critic에게 **동시에**(한 메시지에서 `Agent` 두 번 — lens fan-out과 같은 패턴) 넘긴다. 서로 입력이 겹치지 않고 어느 쪽도 상대 출력에 의존하지 않는다 — 순차로 돌릴 이유가 없다.
   - a. **Completeness critic.** `completeness-critic`에게 `plan` + 사용한 lens 목록을 넘겨 사각지대(아무 lens도 안 본 관심사)를 받는다.
   - b. **Assumption critic.** `assumption-critic`에게 `plan`(+ repo 있으면 경로)을 넘겨, 이 plan이 성립하려면 뭘 전제해야 하고 그 전제가 틀리면 **접근 자체**가 바뀌는지 캔다. a와 다른 축이다 — a는 "안 본 관심사", b는 "숨은 전제"(과거 `direction-explore`가 후보 비교로만 얻던 것을 비교 없이 재현 — README "삭제됨" 참고). 나온 항목은 `## 미해결·검증 필요`에 적는다. plan을 강제로 고치진 않는다 — 사람 또는 다음 라운드 판단 몫.
...
7. **출력 — 결과물과 과정 로그를 분리한다.** "써먹을 plan"(다운스트림 구현용)과 "어떻게 나왔나 기록"(감사·dogfooding)은 소비자가 다르다. 한 파일에 섞지 마라. `<slug>` = plan 제목 기반 또는 사용자 지정, 없으면 `<날짜>-<n>`.
   ...
   **파일로 쓰기 전에 최종 plan을 한 번 다시 읽어라**(너 자신, 서브에이전트 없음 — 비용 0). 여러 라운드 patch를 거치며 남은 잔재 — 앞뒤가 모순된 문장, 미해결 TODO, 모호한 placeholder — 가 있으면 지금 고쳐라. `completeness-critic`(안 본 관심사)·`assumption-critic`(숨은 전제)과 다른 축이다 — 이건 "이 문서가 자기 안에서 앞뒤가 맞나"만 본다.
...
## 비용·정직성 규칙
- 서브에이전트 모델은 정의에 박혀 있다: **판단 lens(`lens-structure`·`lens-scope`·`lens-testability`·`lens-security`)와 `plan-reviser`·`assumption-critic`은 `sonnet`**(오라클 없는 판단이라 약한 모델이 off-lens로 드리프트), **enumeration·추출(`lens-failure`·`lens-cost`·`lens-operability`·`friction-extractor`·`completeness-critic`)은 `haiku`**. 라운드·lens 수를 작게 유지해 사용량을 아껴라.
