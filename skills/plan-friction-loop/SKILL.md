...
이 스킬은 부트스트랩이라 종종 초안 없이 goal 한 줄만 들어온다. 초안을 쓰기 전에 먼저 이 게이트를 거쳐라. **서브에이전트 없이 너(메인 에이전트) 혼자, 비용 0.**
1. 얕은 repo scan(있으면) 먼저 — grep 한 번이면 풀리는 걸 사용자에게 되묻지 마라.
2. 그래도 **goal이 다중 해석이고, 그 해석 차이가 plan의 핵심 구조를 가르는가**만 판정해라. 세부 파라미터 수준 모호함(예: 폴링 주기, 로그 포맷)은 게이트 대상이 아니다 — lens가 알아서 처리한다.
3. 아니면(대부분의 요청) 그냥 초안 한 문단을 써서 바로 아래 절차로 진행해라.
4. 맞으면 `AskUserQuestion`으로 **한 라운드만**(필요하면 한 번에 여러 질문 배치) 물어라 — 답 받고 또 물어보는 왕복 반복 금지. 답으로 goal을 확정한 뒤 초안을 써서 진행해라.
- ⚠️ **단 `auto`가 구동 중이면 이 게이트를 생략**하고, auto의 시작 1회 동의(`AskUserQuestion`)에 명확화 질문을 얹은 것으로 간주하라 — auto는 그 이후 안 멈춘다.
- 이건 **요청 자체의 모호성**만 잡는다. plan을 다 쓴 뒤에야 드러나는 모호성(예: 숨은 전제)은 이 게이트가 아니라 아래 `assumption-critic`(수렴 후) 몫이다 — 둘은 서로 대체가 아니다.
...
6. **Completeness critic.** `completeness-critic`에게 최종 `plan` + 사용한 lens 목록을 넘겨 사각지대(아무 lens도 안 본 관심사)를 받는다.

7. **Assumption critic.** `assumption-critic`에게 최종 `plan`(+ repo 있으면 경로)을 넘겨, 이 plan이 성립하려면 뭘 전제해야 하고 그 전제가 틀리면 **접근 자체**가 바뀌는지 캔다. 6과 다른 축이다 — completeness-critic은 "안 본 관심사", 이건 "숨은 전제"(과거 `direction-explore`가 후보 비교로만 얻던 것을 비교 없이 재현 — README "삭제됨" 참고). 나온 항목은 `## 미해결·검증 필요`에 적는다. plan을 강제로 고치진 않는다 — 사람 또는 다음 라운드 판단 몫.

8. **출력 — 결과물과 과정 로그를 분리한다.** "써먹을 plan"(다운스트림 구현용)과 "어떻게 나왔나 기록"(감사·dogfooding)은 소비자가 다르다. 한 파일에 섞지 마라. `<slug>` = plan 제목 기반 또는 사용자 지정, 없으면 `<날짜>-<n>`.
