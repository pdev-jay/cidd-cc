...
## 규모 적응 (먼저 — work-unit 도출 전에)
build에서 줄이는 건 *분해·병렬·judge* 같은 심의/구조 비용이지 **검증(오라클·conformance)이 아니다** — 검증은 싸고 작은 변경일수록 오라클이 잘 잡는다.
- **규모는 state.md에서 먼저 읽어라(요청보다 우선)**: `handoff.plan→build`의 `미해결·검증필요 항목`이 *검증 바닥*의 grounded 신호다(많음 = 리스크 큼 = 검증 사수). profile 라벨은 출발점, 이 증거 + 실제 plan 범위(다이어그램 unit 수)로 정련. 둘 다 없으면 자가 판정(방향·오류비용·테스트 유무, repo scan 우선).
- **micro**(기계적·국소[로직 불변 또는 단일 함수, 파일 수 무관]·되돌리기 쉬움·오라클 존재): work-unit 분해·foundation·병렬·judge 생략 → 메인이 직접 편집 → 오라클 실행 → green. **conformance는 적대적 서브에이전트 대신 메인 self-check + 오라클 증거로 축소.**
- **small/standard**: unit 수 최소, judge는 진짜 갈리는 unit만. conformance 게이트 유지.
- ⚠️ **검증 바닥(끄지 마라)**: 하드 오라클은 항상. **오류비용 큰 표면**(auth·permission·schema/persistence·public API·dependency/build config·concurrency·crypto·billing·external I/O·device/BLE/protocol·보안 로깅)이면 규모 작아도 conformance 유지 + 최소 standard 승격(작은 diff ≠ 작은 영향).
- **오라클 미배선이면 micro 불가** — 빈 스위트의 가짜 green("Ran 0 tests / OK")도 미배선이다. 우선 **특성 테스트부터 배선**해 검증 바닥(red→green)을 깐 뒤 진행(권장). 불가하면 "검증 안 된 코드" 경고 후에만(절대 규칙).
- **번복**: 영향 커지면 올림(내림은 보수적).

## 절차

### 0. 오라클 탐지 + work-unit 도출
- repo에서 오라클 명령을 찾는다(`package.json` scripts / `pubspec.yaml` / `Makefile` / `pyproject` 등). **탐지는 `cidd:oracle-runner`에 위임 가능** — toolchain detect 결과를 받아 `oracleCmd`로 쓴다. **없으면** "⚠️ 오라클 미배선 — build이 verify를 못 함, '검증 안 된 코드 생성'일 뿐"을 맨 위에 박고 사용자 확인 후에만 진행.
