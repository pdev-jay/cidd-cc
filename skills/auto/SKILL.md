...
한 목표를 받아 lifecycle을 **자율 주행**한다. fdd-auto류지만 CIDD의 차별점: **전진 신호가 자기 평가가 아니라 오라클이다.** (배경: `GOAL.md` 원칙 1, "오토모드 향후 opt-in")
...
1. **시작 1회**: "explore→…→review까지 자율 진행할까?" 동의를 받는다(`AskUserQuestion`). build이 코드를 쓰므로 이 1회 동의가 GOAL "코드쓰기 실행은 별개 go"를 흡수한다. 동의 없으면 단계별 모드로.
