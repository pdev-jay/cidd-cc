...
1. **시작 1회**: "plan→…→review까지 자율 진행할까?" 동의를 받는다(`AskUserQuestion`). build이 코드를 쓰므로 이 1회 동의가 "코드쓰기 실행은 별개 go"를 흡수한다. 동의 없으면 단계별 모드로.
...
3. **review gate pass → `stage: done`** + history + 최종 리포트. 리포트 필수 4요소: ① 한 줄 결과(done/어디서 왜 멈춤) ② 거쳐온 단계 한 줄 요약 ③ **⚠️ 미검증·위험 고지**(`PROD PATH UNVERIFIED: <file:line>` + adequacy 미측정 축 — review의 DISCLOSURE를 *파일에 묻지 말고* 리포트 상단으로) ④ 머지 전 사람이 볼 것 한 줄. ⚠️ **멈춤을 안 거니 ③이 미검증 코드의 유일한 안전망** — 미검증 없으면 "전 경로 검증됨"이라도 항상 출력(빈 줄 = 거짓 안심).
